...
![](https://raw.githubusercontent.com/whn961227/images/master/data/20200914200339.png)

**行式存储**

> 优点

1. 相关的数据是保存在一起，比较符合面向对象的思维，因为一行数据就是一条记录
2. 这种存储格式比较方便进行 INSERT/UPDATE 操作
...
MapReduce 在读取数据的时候需要并行, 这就要求压缩后的文件可以分片读取. 分片对于 MapReduce 任务来说**非常重要**.

> 在考虑如何压缩那些将由 MapReduce 处理的数据时，考虑**压缩格式是否支持分片**是很重要的
